...
def send_login(method, path, *, json_data=None, headers=None):
    url = f"{BASE_URL}{path}"
    try:
        resp = requests.request(method.upper(), url, data=json_data, headers=headers, timeout=8)
        return resp, url, method.upper()
    except (RequestException, Timeout, ConnectionError):
        return None, url, method.upper()
...
def api_login(email, senha):
    ...
    return send_login("POST", "/auth/login", json_data=payload), payload
